...
[Note:](#flag-normalization) that all passed ˋflagNamesˋ passed as ˋflagˋ are a string. Depending on the adapter used _these are normalized_ to be camel cased. This means that whenever a ˋfoo-flag-nameˋ is configured in e.g. LaunchDarkly or splitio it will have to be specified as ˋfooFlagNameˋ. The same applies for a ˋfoo_flag_nameˋ. This is meant to help using flags in an adapter agnostic way. Whenever a flag is otherwise passed in the non-normalized form it is likely to default to ˋfalseˋ which is unintended in most cases.
